...
async def test_grab_file_broken_mime_url(bot):
    ...
    message = await dpytest.message(
        "https://images.unsplash.com/photo-1574144611937-0df059b5ef3e?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8ZnVubnklMjBjYXR8ZW58MHx8MHx8fDA%3D&w=1000&q=80"
    )
    fname = file_helper.grab(message)
...
async def test_grab_file_attachment_jpg(bot):
    ...
    message = await dpytest.message(
        content="",
        attachments=[
            "https://images-ext-2.discordapp.net/external/UYsSEFKavGjDMUVHnZF3B3odIp98mx1_vavpjda4rBM/https/pbs.twimg.com/media/F2X7SLRWIAAon-6.jpg?width=418&height=558"
        ],
    )
    fname = file_helper.grab(message)
    assert fname == f"{constants.Bot.file_cache}F2X7SLRWIAAon-6.jpg"
