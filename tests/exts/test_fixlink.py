...
async def test_twitter(bot):
    ...
    message = await dpytest.message(
        "https://twitter.com/JamesCageWhite/status/1686884877696892929"
    )
...
async def test_tiktok(bot):
    ...
    message = await dpytest.message(
        "https://www.tiktok.com/@handsomejamescage/video/7143936716712217857"
    )
    response = await fixlink(message)
    assert (
        "https://www.vxtiktok.com/@handsomejamescage/video/7143936716712217857"
        in response
    )
